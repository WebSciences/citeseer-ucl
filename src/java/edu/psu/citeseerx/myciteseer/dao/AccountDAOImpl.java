 ...
 * @version $$Rev: 719 $$ $$Date: 2008-09-30 11:18:29 -0400 (Tue, 30 Sep 2008) $$
 */
public class AccountDAOImpl extends JdbcDaoSupport implements AccountDAO {
    ...
    public Account getAccount(String username, String password)
            throws UsernameNotFoundException, DataAccessException {
        List users = usersByNamePassMapping.run(username, password);
        if (users.size() == 0) {
            throw new UsernameNotFoundException("User not found");
        }
        Account user = (Account)users.get(0);
    ...
    public Account getAccountByEmail(String emailAddress) {
        List users = usersByEmailMapping.execute(emailAddress);
        if (users.size() == 0) {
            return null;
        }
        Account user = (Account)users.get(0);
    ...
    public boolean isValidActivationCode(String username, String code) {
        List values = getActivation.run(username, code);
        if (values.size() > 0) {
            return true;
        } else {
            return false;
    ...
    throws DataAccessException {
        List ids = validTicketMapping.execute(ticket);
        if (ids.size() > 0) {
            return true;
        } else {
            return false;
    ...
    private static final String DEF_USERS_BY_USERNAME_QUERY =
        ...
        "internalid, updated from users where userid=?";
    ...
    private static final String DEF_USERS_BY_EMAIL_QUERY =
        "select userid, password, firstName, middleName, lastName, " +
        "email, affil1, affil2, enabled, country, province, webPage, " +
        "internalid, updated from users where email=?";
    ...
    private static final String DEF_USERS_BY_NAMEPASS_QUERY =
        ...
        "updated from users where userid=? and password=?";
    ...
    protected class UsersByNamePassMapping extends UserMapping {
        ...
        public List run(String username, String password) {
    ...
    protected class GetActivation extends MappingSqlQuery {
        ...
        public List run(String username, String code) {
            Object[] params = new Object[] { username, code };
            return execute(params);
        } //- GetActivation.run
    ...
    private final static String DEF_GET_ALL_USERS_QUERY =
    	"select userid, password, firstName, middleName, lastName, " +
        "email, affil1, affil2, enabled, country, province, webPage, " +
        "internalid, updated from users where internalid > ? limit ? ";
    ...
    private final static String DEF_GET_USERS_SINCE_TIME_QUERY =
    	"select userid, password, firstName, middleName, lastName, " +
        "email, affil1, affil2, enabled, country, province, webPage, " +
        "internalid, updated from users where updated >= ? " +
