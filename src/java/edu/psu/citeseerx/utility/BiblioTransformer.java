...
public class BiblioTransformer {
     ...
     * @param doc
     * @param url base URL to see DOC. The string "?doi=" + the DOI's object
    ...
    public static String toCOinS(ThinDoc doc, String url) {
    	...
    	/*
    	 * Add OpenURL ContextObject version (1.0) and defines that this
    	 * object context is about a journal. 
    	 */  
    	buffer.append("ctx_ver=Z39.88-2004");
    	buffer.append("&rft_val_fmt=");
    	buffer.append(encode("info:ofi/fmt:kev:mtx:journal"));
    	// Add the DOI.
    	buffer.append("&rft_id=info:doi/");
    	buffer.append(encode(doc.getDoi()));

    	// At the moment we assume the url is valid
    	...
    	if (doc.getVentype() != null) {
    		buffer.append("&rft.jtitle=");
    		buffer.append(encode(doc.getVenue()));
    	}
    	if (doc.getYear() > 0) {
    	...
    	String ventype = doc.getVentype();
        if (ventype == null) ventype = "unknown";
    ...
    /**
     * Returns a list of percent-encode string representing a ContextObject for
    ...
    public static List<String> toCOinS(List<ThinDoc> tDocList, String url) {
    	...
    	while (it.hasNext()) {
    		ThinDoc doc = it.next();
    		ctxObjects.add(toCOinS(doc, url));
    	}
    	return ctxObjects;
    } //- toCOinS
