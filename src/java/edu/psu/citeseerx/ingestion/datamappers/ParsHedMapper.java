...
public class ParsHedMapper {
     ...
     */
    public static edu.psu.citeseerx.domain.Document map(String xml)
    throws MappingException {
    ...
    protected static void buildDoc(edu.psu.citeseerx.domain.Document doc,
            Element root) {
        ...
        for (Iterator it = rootChildren.iterator(); it.hasNext(); ) {
            ...
            if (child.getName().equalsIgnoreCase("title")) {
                ...
                doc.setDatum(edu.psu.citeseerx.domain.Document.TITLE_KEY, val);
                doc.setSource(edu.psu.citeseerx.domain.Document.TITLE_KEY, src);
            }
            if (child.getName().equalsIgnoreCase("abstract")) {
                String val = SafeText.decodeHTMLSpecialChars(child.getValue());
                doc.setDatum(edu.psu.citeseerx.domain.Document.ABSTRACT_KEY,
                        val);
                doc.setSource(edu.psu.citeseerx.domain.Document.ABSTRACT_KEY,
                        src);
            }
            if (child.getName().equalsIgnoreCase("date")) {
                ...
                try {
                    ...
                    doc.setDatum(edu.psu.citeseerx.domain.Document.YEAR_KEY,
                            val);
                    doc.setSource(edu.psu.citeseerx.domain.Document.YEAR_KEY,
                            src);
            ...
            if (child.getName().equalsIgnoreCase("tech")) {
                ...
                doc.setDatum(edu.psu.citeseerx.domain.Document.TECH_KEY, val);
                doc.setSource(edu.psu.citeseerx.domain.Document.TECH_KEY, src);
            }
        }
    ...
    protected static Author mapAuthor(Element authElt, String src) {
        ...
        List children = authElt.getChildren();
