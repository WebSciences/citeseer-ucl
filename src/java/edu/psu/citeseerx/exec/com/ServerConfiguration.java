...
public class ServerConfiguration {
    ...
    public ServerConfiguration(ConfigurationManager cm) throws Exception {
        manager = cm;
        serverPort = cm.getInt("ObjectServer.serverPort", accessKey);
        poolSize = cm.getInt("ObjectServer.poolSize", accessKey);
        readTimeout = cm.getInt("ObjectServer.readTimeout", accessKey);
        useCompression =
            cm.getBoolean("ObjectServer.useCompression", accessKey);
        compressedBlockSize =
            cm.getInt("ObjectServer.compressedBlockSize", accessKey);
        returnAddress = cm.getString("ObjectServer.returnAddress", accessKey);

        String transferType =
            cm.getString("ObjectServer.objectTransferType", accessKey);
