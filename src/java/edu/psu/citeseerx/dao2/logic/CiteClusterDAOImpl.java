...
implements CiteClusterDAO {
    ...
    public List<ThinDoc> getCitedDocuments(Long clusterid,
            int start, int amount) throws DataAccessException {
        int limit = start+amount;
    ...
    throws DataAccessException {
        ...
        return getCitingPaperIDs.run(paperid);
    } //- getCitingDocumentIDs
    
    public List<String> getCitedDocumentIDs(String paperid)
    throws DataAccessException {
        return getCitedPaperIDs.run(paperid);
    } //- getCitedDocumentIDs
    ...
    private static String DEF_GET_CITING_PAPER_IDS_QUERY =
    	"select citing.id from papers citing, citegraph, papers cited where " +
    	"citegraph.citing = citing.cluster and citegraph.cited = " +
    	"cited.cluster and cited.id =?";
    ...
    } //- class GetCitingPaperIDs
    
    private static String DEF_GET_CITED_PAPER_IDS_QUERY =
    	"select cited.id from papers cited, citegraph, papers citing where " +
    	"citegraph.cited = cited.cluster and citegraph.citing = " +
    	"citing.cluster and citing.id = ?";
