<%@ include file="../shared/IncludeTop2.jsp" %>
 <div id="primary_tabs-n-content" class="clearfix"> <!-- Contains header div -->
  <div id="primary_tabs_container">
   <div id="primary_tabs">
    <ul class="clearfix"><li id="current"><a class="page_tabs remove" href="<c:url value="/stats/articles"/>"><span>Most Cited Articles</span></a></li><li><a class="page_tabs remove" href="<c:url value="/stats/citations"/>"><span>Most Cited Citations</span></a></li><li><a class="page_tabs remove" href="<c:url value="/stats/authors"/>"><span>Most Cited Authors</span></a></li><li><a class="page_tabs remove" href="<c:url value="/stats/venues"/>"><span>Venue Impact Ratings</span></a></li></ul>   <!-- remove extra whitespace by coding in-line lists on one line -->           
   </div> <!-- End primary_tabs -->
  </div> <!-- End primary_tabs_container -->
  
  <div id="primary_content">

   <p class="para4 parafirstletters para_book"><span class="firstletters"><em class="firstletter">Most Cited Computer Science Articles</em></span> generated from documents in the <fmt:message key="app.nameHTML"/> database as of <c:out value="${ gendate }"/>.  This list is automatically generated and may contain errors. The list is generated in batch mode and citation counts may differ from those currently in the <fmt:message key="app.nameHTML"/> database, since the database is continuously updated.</p>
   <div id="introduction">
    <div id="citedlinks" class="formating nofrills_list para2nomargin">
     ...
     <c:forEach var="year" items="${ years }">
      <c:if test="${ year == currentlink }">&#124; <a id="currentlink" href="<c:url value="/stats/articles?y=${year}"/>"><c:out value="${ year }"/></a> </c:if>
      <c:if test="${ year != currentlink }">&#124; <a href="<c:url value="/stats/articles?y=${year}"/>"><c:out value="${ year }"/></a> </c:if>
     </c:forEach>
    </div>              
   </div> <!-- End introduction -->                   
   <div class="information_bar">
    <c:if test="${ ! empty nextPageParams }">
     <a href="<c:url value="/stats/articles?${ nextPageParams }"/>">Next <c:out value="${ pageSize }"/> <img class="mini_icon remove" src="<c:url value="/icons/arrwnxt.gif"/>" alt="View Next Page" /></a>  
    </c:if>
